...
const Box = ({ width, height, elevation, rounded, children, outlined, className, margin, padding, boxRef, style }) => {
  ...
  return (
    <div style={boxStyles} ref={boxRef} className={`${styles.rue_box} ${className}`}>
      {children}
    </div>
  );
};
...
Box.propTypes = {
  ...
  boxRef: PropTypes.oneOfType([PropTypes.func, PropTypes.shape({ current: PropTypes.instanceOf(Element) })]),
};
