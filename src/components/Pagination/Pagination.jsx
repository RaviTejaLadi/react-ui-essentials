...
const Pagination = ({ totalPages, currentPage, setCurrentPage, maxPageNumbersToShow = 5, size }) => {
  const renderPageNumbers = () => {
    ...
    for (let i = startPageNumber; i <= endPageNumber; i++) {
      pageNumbers.push(
        <Button variant={i === currentPage ? "primary" : "light"} size={size} key={i} onClick={() => setCurrentPage(i)}>
          {i}
        </Button>
      );
    }
  ...
  return (
    <div className={styles.pagination}>
      ...
      <Button variant="light" size={size} onClick={() => setCurrentPage(currentPage - 1)} disabled={currentPage === 1}>
