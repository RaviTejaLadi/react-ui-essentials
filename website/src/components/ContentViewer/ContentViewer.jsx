...
const ContentViewer = ({ data, header, width, height, margin }) => {
  ...
  const renderSidebarItems = (items) => {
    return items.map((item) => (
      <li key={item.id} className={`${styles.rue_sidebar_item} ${selectedItem === item.label ? styles.active : ""}`}>
        <div
        ...
        >
          ...
          <span className={styles.rue_label_text}>
            {item.type === "folder" ? (
              ...
              <File width="18px" height="18px" style={{ marginRight: "4px", marginBottom: "-5px" }} />
            )}
        ...
        {item.children && expandedItems.includes(item.id) && (
          <ul className={styles.rue_nested_items}>
            <code>{renderSidebarItems(item.children, true)}</code>
          </ul>
        )}
      </li>
    ));
