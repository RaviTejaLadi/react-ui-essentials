  ...
  return (
    <Box elevation={1} rounded margin="10px">
      ...
      <Box
        ...
        style={{ overflow: "auto" }}
      >
        <table className={styles.rue_dynamic_table}>
          <thead className={styles.rue_dynamic_table_thead}>
            <tr className={styles.rue_dynamic_table_tr}>
              {tableHeaders.map((header, index) => (
                <th className={styles.rue_dynamic_table_th} key={`${header}-${index}`}>
                  <Box width="100%" className={styles.rue_dynamic_table_header_edit_options}>
                    {isEditing ? (
                      <input
                        className={styles.rue_dynamic_table_input}
                        type="text"
                        value={header}
                    ...
                    {isEditing && (
                      <Box className={styles.rue_dynamic_table_column_controls_btn}>
                        <Button
                          size="sm"
                          variant="light"
          ...
          <tbody className={styles.rue_dynamic_table_tbody}>
            {rowData.map((row, rowIndex) => (
              <tr
                className={styles.rue_dynamic_table_tr}
                key={`${row}-${rowIndex}`}
              ...
              >
                {row.map((data, columnIndex) => (
                  <td className={styles.rue_dynamic_table_td} key={`${data}-${columnIndex}`}>
                    {isEditing ? (
                      <input
                        className={styles.rue_dynamic_table_input}
                        type="text"
                ...
                {isEditing && (
                  <td className={styles.rue_dynamic_table_td}>
                    <Box className={styles.rue_dynamic_table_row_controls_btn}>
                      <Button
                        size="sm"
                        variant="light"
